...
public class CustomUserDetailsService implements UserDetailsService{
	...
	public UserDetails loadUserByUsername(String ssoId)
		...
		if(usuario==null){
			...
			return new org.springframework.security.core.userdetails.User(usuario.getSsoId(), usuario.getPassword(), 
	...
	private List<GrantedAuthority> getGrantedAuthorities(Usuario usuario){
		...
		for(UsuarioPerfil usuarioPerfil : usuario.getUsuarioPerfil()){
			logger.info("UserProfile : {}", usuarioPerfil);
			authorities.add(new SimpleGrantedAuthority("ROLE_"+usuarioPerfil.getType()));
		}
